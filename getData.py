...
def getDistribution(output_df):
    try:
        fit = base.invisible(gamlss.fitDist(output_df["EMC"].values, k = 2, type = "realplus", trace = False, try_gamlss = True))
...
def getMetaData(output_df, original, fileName, dirName, analyte, flow):
    ...
    metaData_df.to_csv(os.path.join(LocAnalyteMetaData, dirName + "_" + fileName + "_" + flow + "_" + analyte.lower() + ".csv"), index = False)
...
def resampleData(df, type, dirName, fileName, analyte):
    ...
    for sampleSize in sampleSizes:
        ...
        resampled.to_csv(os.path.join(ResampledLocAnalyte, dirName + "_" + fileName + "_" + type + "_" + analyte.lower() + "_resampled_" + str(sampleSize) + ".csv"), index = False)

def getInflow(df, dirName, fileName, analyte, names):
    original_inflow = df[df["msname"].isin(names)]
    inflow = df[(df["msname"].isin(names)) & (df["initialscreen_flag"] != "No")]
    ...
    if inflow.empty:
        ...
        if len(output_df) < 15:
            ...
            getMetaData(output_df, original_inflow, fileName, dirName, analyte, "inflow")
            LocAnalyte = os.path.join(outputPath, fileName + "_" + analyte)
            if not os.path.exists(LocAnalyte):
                os.makedirs(LocAnalyte)

            output_df.to_csv(os.path.join(LocAnalyte, dirName + "_" + fileName + "_inflow_" + analyte.lower() + ".csv"), index = False)
...
def getOutflow(df, dirName, fileName, analyte, names):
    ...
    outflow = df[(df["msname"].isin(names)) & (df["initialscreen_flag"] != "No")]
    ...
    if outflow.empty:
        ...
        if len(output_df) < 15:
            ...
            getMetaData(output_df, original_outflow, fileName, dirName, analyte, "outflow")
            LocAnalyte = os.path.join(outputPath, fileName + "_" + analyte)
            if not os.path.exists(LocAnalyte):
                os.makedirs(LocAnalyte)

            output_df.to_csv(os.path.join(LocAnalyte, dirName + "_" + fileName + "_outflow_" + analyte.lower() + ".csv"), index = False)
...
def createDataframes():
    ...
    in_names = pd.read_csv(os.path.join(locationsDataPath, "in_msnames.csv"), names = ["InflowName"])["InflowName"].to_list()
    out_names = pd.read_csv(os.path.join(locationsDataPath, "out_msnames.csv"), names = ["OutflowName"])["OutflowName"].to_list()
        ...
        if int(dirName) in desiredDirs:
            ...
            for file in files:
                ...
                if phosphorusSheet.size != 0:
                    ...
                    getInflow(phosphorus, dirName, fileName, "Phosphorus", in_names)
                    getOutflow(phosphorus, dirName, fileName, "Phosphorus", out_names)
...
shutil.make_archive(os.path.join(os.getcwd(), exportPath, "OutputData"), "zip", outputPath)
shutil.make_archive(os.path.join(os.getcwd(), exportPath, "ResampledData"), "zip", resampledPath)
shutil.make_archive(os.path.join(os.getcwd(), exportPath, "MetaDataData"), "zip", metadataPath)
